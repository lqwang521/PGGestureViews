...
- (void)validatePassword
{
    ...
    if (isSave) {
        ...
        setpass.forgetPasswordBlock = ^(NSString *string) {
            
            NSDictionary *me = @{};//[[MXKit shareMXKit] getCurrentUser];
            NSDictionary *passCodeEnableDic = [[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithBool:NO],@"gesturePassCode", [NSString stringWithFormat:@"%@",[me objectForKey:@"account_id"]],@"userid", nil];
            NSData *passCodeData = [NSKeyedArchiver archivedDataWithRootObject:passCodeEnableDic];
            //[[MXKit shareMXKit] setMXUserDefaultBinaryValue:passCodeData forKey:@"gesturePassCode"];
            NSString *tempString = [[NSString alloc] initWithData:passCodeData encoding:NSUTF8StringEncoding];
 
            
        };
        setpass.logoutBlock = ^(NSString *string) {
