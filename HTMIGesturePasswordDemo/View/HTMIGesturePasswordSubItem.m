...
#define HTMIGesturePasswordSUBITEMTAG 33

@interface HTMIGesturePasswordSubItem()
//@property(nonatomic , strong)CAShapeLayer *shape;
@end


@implementation HTMIGesturePasswordSubItem
...
- (void)initViews
{
    for (int i=0; i<9; i++)
    {
        ...
        myView.tag = i + HTMIGesturePasswordSUBITEMTAG;
...
- (void)resultArr:(NSArray *)array fillColor:(UIColor *)color
{
    [self.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        {
            // 如果array里包含idx，填充为白色
            UIView *myView = (UIView *)[self viewWithTag:(idx+HTMIGesturePasswordSUBITEMTAG)];
